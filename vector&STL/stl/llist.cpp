...
int main(){
    ...
    first.insert(first.begin(), 0); // Insert 0 at the beginning
    first.insert(first.end(), 4); // Insert 4 at the end

    list<int>::iterator it = first.begin(); // Initialize the iterator to the beginning of the list
    cout << "Elements in the list: ";

    while(it != first.end()) { // Loop until the end of the list
        cout << *it << " "; // Dereference the iterator to get the value
        it++; // Move to the next element
    }
    ...
    cout<<first.size()<<endl; // Size of the list
    cout << endl;
    first.erase(first.begin(), first.end()); // Erase the first element
   cout<<first.size()<<endl; // Size of the list
    cout << endl;
